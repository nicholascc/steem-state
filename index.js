...
module.exports = function(client, steem, currentBlockNumber='currentBlock', blockComputeSpeed=1000, prefix='') {
  ...
  function beginBlockComputing() {
    function computeBlock() {
      ...
      var blockNum = currentBlockNumber;// Helper variable to prevent race condition
                                        // in getBlock()
      client.database.getBlock(currentBlockNumber)
        .then((result) => {
  ...
  function beginBlockStreaming() {
    var stream = client.blockchain.getBlockStream({mode: steem.BlockchainMode.Latest});
  ...
  function processBlock(block, num) {
    onNewBlock(currentBlockNumber, block);
